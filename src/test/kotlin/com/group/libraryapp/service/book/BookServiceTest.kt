...
import com.group.libraryapp.domain.user.loanhistory.UserLoanHistory.Companion.fixtrue
...
)
{
    ...
    @Test
    @DisplayName("책이 이미 대출상태라면 실패한다.")
    fun loanBookFailTest(){
        bookRepository.save(Book.fixture("이상한 나라의 엘리스"))
        val savedUser=userRepository.save(User("최태현",null))

        userLoanHistoryRepository.save(fixtrue(savedUser,"이상한 나라의 엘리스"))
        val request=BookLoanRequest("최태현","이상한 나라의 엘리스")
    ...
    fun countLoanedBookTest(){
        ...
        userLoanHistoryRepository.saveAll(
            listOf(
                UserLoanHistory.fixtrue(savedUser,"A"),
                UserLoanHistory.fixtrue(savedUser,"B",UserLoanStatus.RETURNED)
            )
        )
